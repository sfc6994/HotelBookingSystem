...
public class RoomDAO {
    ...
    public void addRoom(Room room) throws SQLException {
        try (PreparedStatement pstmt = conn.prepareStatement("INSERT INTO ROOMS(ROOMNUMBER, ROOMTYPE, CAPACITY, STATUS) VALUES (?, ?, ?, ?)")) {
    ...
    public Room findRoom(int roomNumber) throws SQLException {
        Room room;
        try (PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM ROOMS WHERE ROOMNUMBER = ?")) {
    ...
    public void updateRoomStatus(int roomNumber, RoomStatus status) throws SQLException {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement("UPDATE ROOMS SET STATUS = ? WHERE ROOMNUMBER = ?")) {
    ...
    public ArrayList<Room> getAllRooms() throws SQLException {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM ROOMS"); ResultSet rs = pstmt.executeQuery()) {
            rooms = new ArrayList<>();
            while (rs.next()) {
                Room room = new Room(
                        rs.getInt("ROOMNUMBER"),
